           ...
           //logic to check if correct user id and password entered - Start()
            if (isset($_POST['login']) && !empty($_POST['email']) 
               && !empty($_POST['password'])) {
                ...
                $result = @mysqli_query($dbc, "select count(1) from admin where email='$email' and pass='$password'");
               ...
               if ($noOfrecords>0) {
				  ...
				  $result = @mysqli_query($dbc, "select admin_id from admin where email='$email' and pass='$password'");
				  $row = mysqli_fetch_array($result, MYSQLI_NUM);
				  $noOfrecords=$row[0];
